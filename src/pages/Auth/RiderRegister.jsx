...
export default function RiderRegister() {
  ...
  const handleSubmit = e => {
    ...
    // Register rider (not verified yet)
    mockUsers.push({
      fullName,
      phone,
      email,
    ...
    setSuccess(true);
    setTimeout(() => navigate('/rider-verification'), 1500);
  };

  return (
    <Box className="max-w-md mx-auto p-6 bg-white rounded shadow mt-10">
      <Typography variant="h5" className="mb-4 font-bold">Rider Registration</Typography>
      {error && <Alert severity="error" className="mb-3">{error}</Alert>}
      {success && <Alert severity="success" className="mb-3">Registration successful! Redirecting to verification...</Alert>}
